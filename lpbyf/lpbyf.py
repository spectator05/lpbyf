...
class Lpbyf:
    ...
    def set_label_dict(self, csv_path: str, header_dict: dict=None):
        ...
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                key = '_'.join([str(row[sa_idx]), str(row[da_idx]), str(row[sp_idx]), str(row[dp_idx]), str(row[prtcl_idx]).lower()])
                if not (key in self.label_dict):
                    self.label_dict[key] = {}
                self.label_dict[key][float(row[ts_idx])] = [float(row[te_idx]), row[label_idx]]
    ...
    def get_next_label(self, timestamp:float, sa:str, da:str, sp:str, dp:str, prtcl:str) -> str:
        prtcl = prtcl.lower()
    ...
    class Splitter:
        def __init__(self, pcap_list: list, num_core: int = 1, target_size: int = 1000000000, split_packet_count = 10000000, output_path: str = tempfile.gettempdir()):
        ...
        def _split_pcap_with_editcap(self, input):
            input_path, output_path, split_packet_count, editcap_path, keep_files = input
            if not output_path:
            ...
            for output_file in output_files:
                if input_filename in os.path.basename(output_file):
                    pcap_files.append(os.path.join(output_path, os.path.basename(output_file)))
        ...
        def _get_splitted_pcap(self, path_list: list):
            ...
            with mp.Pool(self.num_core) as pool:    
                for pkts_list in pool.imap_unordered(   self._split_pcap_with_editcap,
                                                        zip(path_list,
                                                        repeat(self.output_path),
                                                        repeat(self.split_packet_count),
                                                        repeat(self.editcap_path),
                                                        repeat(self.keep_files)), chunksize=1):
